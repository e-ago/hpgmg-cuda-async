...
def main():
    ...
    cf.add_argument('--CXX', help='Path to C++ compiler', default=os.environ.get('CXX',''))
    cf.add_argument('--CFLAGS', help='Flags for C compiler', default=os.environ.get('CFLAGS',''))
    cf.add_argument('--CXXFLAGS', help='Flags for C++ compiler', default=os.environ.get('CXXFLAGS',''))
...
#    fv.add_argument('--fv-cuda', action='store_true', dest='fv_cuda', help='Use CUDA implementation for the Finite-Volume solver')
...
def makefile(args):
    ...
    m = ['HPGMG_ARCH = %s' % args.arch,
         ...
         'HPGMG_CXXFLAGS = %s' % args.CXXFLAGS,
