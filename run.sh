...
$MPI_HOME/bin/mpirun -verbose     -x PATH    -x GDS_CQ_WAIT_SMART=$var_GDS_WAIT_SMART -x GDS_ENABLE_DEBUG=0    -x MP_ENABLE_DEBUG=0   -x MP_EVENT_ASYNC     -x MP_ENABLE_WARN     -x MP_GUARD_PROGRESS     -x CUDA_VISIBLE_DEVICES     -x LD_LIBRARY_PATH     -x SIZE     -x MAX_SIZE     -x KERNEL_TIME     -x CALC_SIZE     -x COMM_COMP_RATIO     -x USE_SINGLE_STREAM     -x USE_GPU_ASYNC \
